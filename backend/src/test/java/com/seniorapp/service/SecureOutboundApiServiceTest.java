...
class SecureOutboundApiServiceTest {
    ...
    void setUp() {
        ...
        // Mock successful decryption by default (lenient to allow unused stubs)

        lenient().when(cryptoService.decrypt(VALID_ENCRYPTED_TOKEN)).thenReturn(VALID_PLAIN_TOKEN);



        // Mock successful API response by default (lenient to allow unused stubs)
        ...
        // SecureLogger is now a @Mock, so it will automatically do nothing for unconfigured calls

...
function ProfessorDashboard() {
  const [requests, setRequests] = useState([
    { requestId: 'req-888', groupId: 'grp-98765', groupName: 'Tech Titans', members: ['Efkan', 'John', 'Jane'] }
  ]);

  const handleDecision = async (groupId, requestId, decision) => {
    try {
      await fetch(`http://localhost:3000/api/groups/${groupId}/advisor-requests/${requestId}`, {
        method: 'PUT',
  ...
  return (
    <div style={{ backgroundColor: '#ffffff', padding: '30px', borderRadius: '16px', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)', marginBottom: '30px' }}>
      ...
      {requests.length === 0 ? (
        ...
        requests.map((req) => (
          <div key={req.requestId} style={{ backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', padding: '20px', borderRadius: '12px', marginTop: '20px' }}>
